...
public class TST<Value> {
    ...
    **************************************************************/
    ...
    public static void main(String[] args) throws Exception{
    	...
    	reader = new BufferedReader(new FileReader("D:\\Adv Comp Assignments\\Data Files\\Protein.txt"));
    	...
    	while (currentLine != null) 
        {
            ...
            {
            	 token = st.nextToken();
            	 //int count = tst.get(token);
            	 if (tst.get(token)!= null && tst.get(token) >=1 )
            	 {
            		 int count = tst.get(token);
            		 tst.put(token,count+1);
            	 }
            	 else 
            	 {
            		 tst.put(token,1);
    	...
    	reader.close();
        String keys[] = {"protein","complex","prediction","hydrophobic", "PPI"};  
        for (String str : keys)
        {
        	 ;//StdOut.println("key ="+ str + " , value = " + tst.get(str));
        }
    }
}